...
class ExpCall extends Tree.Stm {
  ...
  public Tree.Stm build(Tree.ExpList kids) {
	return new Tree.EXPE(call.build(kids));
  }
@Override
public void accept(CodeVisitor v) {
	// TODO Auto-generated method stub
...
public class Canon {
 ...
 static boolean isNop(Tree.Stm a) {
   return a instanceof Tree.EXPE
          && ((Tree.EXPE)a).exp instanceof Tree.CONST;
 }
 ...
 static Tree.Stm do_stm(Tree.EXPE s) { 
 ...
 static Tree.Stm do_stm(Tree.Stm s) {
     ...
     else if (s instanceof Tree.EXPE) return do_stm((Tree.EXPE)s);
     else return reorder_stm(s);
 }
 ...
 static StmExpList nopNull = new StmExpList(new Tree.EXPE(new Tree.CONST(0)),null);
